...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint, num_of_group=0):
  ...
  for name in name_to_variable:
    if name in init_vars_name:
      tvar_name = name
    elif (re.sub("/group_\d+/", "/group_0/", name) in init_vars_name and
          num_of_group > 1):
      tvar_name = re.sub("/group_\d+/", "/group_0/", name)
    elif (re.sub("/ffn_\d+/", "/ffn_1/", name) in init_vars_name and
          num_of_group > 1):
      tvar_name = re.sub("/ffn_\d+/", "/ffn_1/", name)
    elif (re.sub("/attention_\d+/", "/attention_1/", name) in init_vars_name and
          num_of_group > 1):
      tvar_name = re.sub("/attention_\d+/", "/attention_1/", name)
    else:
